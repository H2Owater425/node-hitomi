...
describe("Gallery", function (): void {
  test("getThumbnails returns first and middle files", function (): void {
    ...
    const translationLanguage: Language = new Language(
      hitomi,
      "english",
      "English",
    );
    const translationId: number = 234567;
    const translation: TranslatedGallery = new TranslatedGallery(
      hitomi,
      translationId,
      translationLanguage,
      "/galleries/" + translationId + ".html",
    );
    ...
    assert.deepStrictEqual(thumbnails, [
      files[0],
...
describe("GalleryManager", function (): void {
  test("retrieve builds a full Gallery from response payload", async function (): Promise<void> {
    ...
    const hitomi: Hitomi = createMock<Hitomi>({
      ...
      request: function (url: URL, range?: string): Promise<Buffer> {
        ...
        return Promise.resolve(
          Buffer.from("var galleryinfo = " + JSON.stringify(rawGallery)),
        );
      },
    });
    ...
    assert.strictEqual(
      gallery["series"]["length"],
      rawGallery["parodys"]["length"],
    );
    ...
    assert.strictEqual(
      gallery["characters"]["length"],
      rawGallery["characters"]["length"],
    );
    ...
    for (let i: number = 0; i < gallery["files"]["length"]; i++) {
      // @ts-expect-error
      assert.ok(
        gallery["files"][i]["hasAvif"] ===
          Boolean(rawGallery["files"][i]["hasavif"]),
      );
      assert.ok(
        gallery["files"][i]["hasThumbnail"] ===
          (i === 0 || i === thumbnailIndex),
      );
    }

    assert.strictEqual(
      gallery["translations"]["length"],
      rawGallery["languages"]["length"],
    );

    for (let i: number = 0; i < gallery["translations"]["length"]; i++) {
      assertInstanceOf(gallery["translations"][i], TranslatedGallery);
      assert.strictEqual(
        gallery["translations"][i]["id"],
      ...
      // @ts-expect-error
      assert.strictEqual(
        gallery["translations"][i]["language"]["name"],
        rawGallery["languages"][i]["name"],
      );
      // @ts-expect-error
      assert.strictEqual(
        gallery["translations"][i]["language"]["localName"],
        rawGallery["languages"][i]["language_localname"],
      );
      assert.strictEqual(
        gallery["translations"][i]["url"],
        rawGallery["languages"][i]["url"],
      );
    }

    assert.strictEqual(
      gallery["relations"]["length"],
      rawGallery["related"]["length"],
    );

    for (let i: number = 0; i < gallery["relations"]["length"]; i++) {
      assertInstanceOf(gallery["relations"][i], GalleryReference);
      assert.strictEqual(
        gallery["relations"][i]["id"],
        rawGallery["related"][i],
      );
    }
    ...
    assert.strictEqual(
      gallery["publishedDate"]?.toISOString(),
    ...
    assert.strictEqual(
      ...
      rawGallery["files"][1]["height"],
    );
    assert.strictEqual(
      gallery["video"]["width"],
      rawGallery["files"][1]["width"],
    );
    assert.strictEqual(
      gallery["video"]["url"],
      "//streaming." + BASE_DOMAIN + "/videos/" + rawGallery["videofilename"],
    );
    assert.strictEqual(
      ...
      "//a." +
        BASE_DOMAIN +
        "/videos/posters/" +
        rawGallery["files"][1]["hash"].slice(-1) +
        "/" +
        rawGallery["files"][1]["hash"].slice(-3, -1) +
        "/" +
        rawGallery["files"][1]["hash"] +
        ".webp",
    );
  });
  ...
  test("list rejects page with multiple non-language tags", async function (): Promise<void> {
    ...
    const manager: GalleryManager = new GalleryManager(hitomi);
    const tags: Tag[] = [
      new Tag(hitomi, "artist", "artist tag"),
      new Tag(hitomi, "group", "group tag"),
    ];
  ...
  test("list returns paginated references for tag query", async function (): Promise<void> {
    ...
    assert.deepStrictEqual(calls, [
      {
        url: ["ltn.gold-usergeneratedcontent.net", "/n/artist/john%20doe-all"],
        range: "2-3",
      },
    ]);
    assert.deepStrictEqual(
      references.map(function (reference: GalleryReference): number {
        return reference["id"];
      }),
      [11, 22],
    );
