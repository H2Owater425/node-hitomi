...
export function getGalleryIdList(range: { startIndex: number; endIndex?: number; }, option?: { orderCriteria?: OrderCriteria, reverse?: boolean; }): Promise<number[]> {
	if(!isInteger(range['startIndex']) || (isInteger(range['startIndex']) && range['startIndex'] < 0)) {
		...
		return new Promise<number[]>(function (resolve: (value: number[] | PromiseLike<number[]>) => void, reject: (reason: any) => void) {
			try {
				...
				fetch(`https://ltn.hitomi.la/${orderCriteria}-all.nozomi`, {
					...requestOption,
					headers: {
