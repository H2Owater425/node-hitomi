...
export class Language extends Base {
   ...
   */
  public toTag(is_negative: boolean = false): Tag {
    return new Tag(this["hitomi"], "language", this["name"], is_negative);
  }
}
...
export class Tag extends Base {
  ...
  constructor(
     ...
     * @readonly
     */
    public readonly is_negative: boolean = false,
  ) {
    super(hitomi);
  ...
  public async listLanguages(): Promise<Language[]> {
        ...
        for (; i < BINARY_ORDERED_LANGUAGES["length"]; i++) {
          if (this["name"] === BINARY_ORDERED_LANGUAGES[i][0]) {
            return [
              new Language(
                this["hitomi"],
                BINARY_ORDERED_LANGUAGES[i][0],
                BINARY_ORDERED_LANGUAGES[i][1],
              ),
            ];
          }
        }
    ...
    const rootNode: Node | undefined = await this["hitomi"][
      "languageIndex"
    ].getNodeAtAddress(0n, version);

    if (!rootNode) {
      throw HitomiError["ROOT_NODE_EMPTY"];
    }

    const data: Node[1][number] | undefined = await this["hitomi"][
      "languageIndex"
    ].binarySearch(hashTerm(term), rootNode, version);
    ...
    for (
      let mask: bigint = 1n;
      i < BINARY_ORDERED_LANGUAGES["length"];
      i++, mask <<= 1n
    ) {
      if (data[0] & mask) {
        languages.push(
   ...
   */
  public toString(is_negative: boolean = this["is_negative"]): string {
    return (
      (is_negative ? "-" : "") +
      this["type"] +
      ":" +
...
export class TagManager extends Base {
  ...
  public parse(expression: string): Tag[] {
    ...
    while ((nextIndex = expression.indexOf(" ", currentIndex)) !== -1) {
      ...
      if (colonIndex !== -1 && colonIndex < nextIndex) {
        ...
        const is_negative: Tag["is_negative"] =
        ...
        if (!rawTags.has(rawTag)) {
          tags.push(
            new Tag(
              this["hitomi"],
              expression.slice(
                currentIndex + (is_negative as unknown as number),
                colonIndex,
              ) as Tag["type"],
  ...
  public async search(term: string): Promise<[Tag, number][]> {
    const is_negative: boolean = term[0] === "-";
    let i: number = term.indexOf(":") + 1;
    let path: string = "";

    if (i) {
      path = "/" + term.slice(is_negative as unknown as number, i - 1);
    } else if (is_negative) {
      i++;
    }
    ...
    const response: [string, number, Tag["type"]][] = JSON.parse(
      String(await this["hitomi"].request([TAG_INDEX_DOMAIN, path + ".json"])),
    );
      ...
      default: {
        if (!startsWith) {
          throw new HitomiError(
            "StartsWith",
            "provided except for language and type",
          );
        }
        ...
        while (
          (currentIndex =
            response.indexOf(target, nextIndex) + target["length"]) !== endIndex
        ) {
          nextIndex = response.indexOf(".", currentIndex);

          if (
            type !== "tag" ||
            (!response.startsWith("male", currentIndex) &&
              !response.startsWith("female", currentIndex))
          ) {
            tags.push(
              new Tag(
                this["hitomi"],
