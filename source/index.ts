...
module hitomi {
    ...
    async function getImagePathCode(): Promise<string> {
        //TODO: return imagePathCode from ltn.hitomi.la/gg.js
        const headers: Headers = new Headers();
        headers.set('Referer', 'https://hitomi.la')
        const code = JSON.parse((await (await fetch('https://ltn.hitomi.la/gg.js', {
            headers: headers
        })).text()).split('gg = ')[0].split('};')[1] + '}');
        return code.b;
    }

    async function getImageSubdomainRegularExpression(): Promise<RegExp> {
        //TODO: return imageSubdomainRegularExpression from ltn.hitomi.la/gg.js
        const headers: Headers = new Headers();
        headers.set('Referer', 'https://hitomi.la')
        const code = await (await fetch('https://ltn.hitomi.la/gg.js', {
            headers: headers
        })).text();
        code.match(/case [0-9]+:/g)
    }
