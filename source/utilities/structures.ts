...
export class HitomiError extends Error {
  ...
  public static get TAG_TYPE(): HitomiError {
    return new HitomiError(
      "Type",
      "one of artist, group, type, language, series, character, male, female, tag",
    );
  }
  // @internal
...
export class IndexProvider extends Provider<string> {
  constructor(
    ...
    super(
      hitomi,
      async function (this: IndexProvider): Promise<string> {
        return String(
          await this["hitomi"].request([
            RESOURCE_DOMAIN,
            "/" + this["field"] + "index/version",
          ]),
        );
      },
  ...
  public async getNodeAtAddress(
    address: Node[2][number],
    version: string,
  ): Promise<Node | undefined> {
    const response: Buffer = await this["hitomi"].request(
      [
        RESOURCE_DOMAIN,
        "/" +
          this["field"] +
    ...
    for (i = 0; i < dataCount; i++) {
      node[1].push([
        response.readBigUInt64BE(offset),
        response.readInt32BE(offset + 8),
      ]);
  ...
  public async binarySearch(
    key: any,
    node: Node,
    version: string,
  ): Promise<Node[1][number] | undefined> {
    if (!node[0]["length"]) {
      return;
    }
    ...
    const nextNode: Node | undefined = await this.getNodeAtAddress(
      node[2][index],
      version,
    );
