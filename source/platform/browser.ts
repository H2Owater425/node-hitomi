...
// @internal
export function createContext(host: string, path: string, type: ResponseType, range?: string): RequestContext<RequestInit> {
	return {
		host: host,
		path: path,
		type: type,
...
export async function request(this: Hitomi, host: string, path: string, type: ResponseType, range?: string): Promise<unknown> {
	let context: RequestContext<RequestInit> = createContext(host, path, type, range);
